function [] = training_Israel2(subjectID,order,mainPath,test_comp,runInd,total_num_runs_training,Ladder1IN,Ladder2IN)
...
% as possible after seeing the cue.
...
% set the maximal time for cue presentation, in seconds
CueDuration=0.1796;
...
images=dir('./stim/*.jpg');
theImage=imread(['./stim/',images(1).name]);
...
%     % Scale the size of the cue as a function of the size of the cue itself
...
if test_comp == 1;
    CenterText(w,'Please focus on the images onscreen when you see the cue.',white,0,-220);
...
for runNum = runInd:total_num_runs_training %this for loop allows all runs to be completed
    ...
    fprintf(fid1,'subjectID\t order\t runNum\t itemName\t onsetTime\t shuff_trialType\t RT\t respInTime\t CueTime\t response\t fixationTime\t ladder1\t ladder2\t bidIndex\t itemNameIndex\t bidValue\t \n'); %write the header line
    ...
    %---------------------------
    Cue_time{runNum}(1:length(shuff_trialType{runNum}),1) = 999;
    ...
    Images_texture= cell(1, length(shuff_names{runNum}));
    for i = 1:length(shuff_names{runNum})
        Images{i} = imread(['stim/',shuff_names{runNum}{i}]);
        Images_texture{i}=Screen('MakeTexture', w, Images{i});
    end
    ...
    for trialNum = 1:length(shuff_trialType{runNum})   % To cover all the items in one run.
        ...
        Screen('DrawTextures',w,Images_texture{trialNum});
        ...
        while (GetSecs-image_start_time < image_duration)
            ...
            %High-Valued GO items
            %---------------------------
            if  shuff_trialType{runNum}(trialNum) == 11 && (GetSecs - image_start_time >= Ladder1{runNum}(length(Ladder1{runNum}),1)/1000 - flip_interval) % shuff_trialType contains the information if a certain image is a GO/NOGO trial
                ...
                if (GetSecs - image_start_time <= (Ladder1{runNum}(length(Ladder1{runNum}),1)/1000 + CueDuration) - flip_interval) && cue_was_presented == 0;
                    % "Visual Cue!"
                ...
                if (GetSecs - image_start_time > (Ladder1{runNum}(length(Ladder1{runNum}),1)/1000 + CueDuration - flip_interval))
                    % turn off visual cue
                    % Draw the background image
                    Screen('DrawTextures',w,Images_texture{trialNum});
                    % Flip to the screen
                ...
                %Low-Valued Go items
                %---------------------------
            elseif  shuff_trialType{runNum}(trialNum) == 22 && (GetSecs - image_start_time >= Ladder2{runNum}(length(Ladder2{runNum}),1)/1000 - flip_interval)  %shuff_trialType contains the information if a certain image is a GO/NOGO trial
                ...
                if (GetSecs - image_start_time <= (Ladder2{runNum}(length(Ladder2{runNum}),1)/1000 + CueDuration) - flip_interval) && cue_was_presented==0
                    % "Visual Cue!"
                    % Draw the background image
                    Screen('DrawTextures',w,Images_texture{trialNum});
                    % Draw the Cue image
                ...
                if (GetSecs - image_start_time > (Ladder2{runNum}(length(Ladder2{runNum}),1)/1000 + CueDuration) - flip_interval)
                    % turn off visual cue
                    % Draw the background image
                    Screen('DrawTextures',w,Images_texture{trialNum});
                    % Flip to the screen
        ...
        switch respInTime{runNum}(trialNum,1)
            case 1 % didn't respond even after 1500 msec on HV GO trial - make it easier decrease SSD by step
                if (Ladder1{runNum}(length(Ladder1{runNum}),1)<50)
            ...
            case 2 % didn't respond even after 1500 msec on LV GO trial - make it easier decrease SSD by step
                if (Ladder2{runNum}(length(Ladder2{runNum}),1)<50)
    ...
    % either before (11,22) or after (110,220)the Cue (but before the
    % image disappeared)
    ...
    if runNum ~= total_num_runs_training && firstOrSecond == 0 && runNum ~= runInd % if this is not the last run, but it is an even run and there was a run before (runNum~=runInd), display instructions again for the next run
        ...
        if goodTrials < goTrials/2
            ...
            %             CenterText(w,strcat(sprintf('You responded on %.2f', (correct{runNum}(1))/20*100), '% of Cue trials'), white, 0,-300);
        ...
        CenterText(w,'Press the button b when you see the cue.',white,0,-270);
...
clear Images Instructions_image CueImage CueImageAlpha;
