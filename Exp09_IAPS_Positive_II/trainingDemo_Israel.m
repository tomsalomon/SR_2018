...
demo_images_texture = cell(1,length(demoNames));
    for i = 1:length(demoNames)
        demo_images{i} = imread(sprintf('stim/demo/%s',demoNames{i}));
        demo_images_texture{i} = Screen('MakeTexture', w, demo_images{i});
    end
...
if test_comp == 1;
    ...
    CenterText(w,'Please focus on the image onscreen when see the cue.',white,0,-220);
...
for runNum = 1:numRuns
    ...
    fprintf(fid1,'subjectID\t runNum\t itemname\t onsettime\t trialtype\t RT\t respInTime\t Cuetime\t response\t fixationtime\t ladder1\t ladder2\n'); %write the header line
...
%     load('Misc/demo_go.mat');
    ...
    cue_time{runNum}(1:length(trialType{1}),1) = 999;
    ...
    for Pos = 1:length(demoNames) %length(stop{runNum}),   % To cover all the items in one run.
        ...
        Screen('DrawTextures',w,demo_images_texture{Pos});
        ...
        while (GetSecs-image_start_time < image_duration),
            ...
            if  demo_go(Pos) == 11 && (GetSecs - image_start_time >= Ladder1{runNum}(length(Ladder1{runNum}),1)/1000) && nocue %demo_go contains the information if a certain image is a demo_go trial or not
                % "Visual Beep!"
                ...
                Screen('Flip', w);
                nocue = 0;
                cue_time{runNum}(Pos,1) = GetSecs - image_start_time;
                ...
                %---------------------------
            elseif  demo_go(Pos) == 22 && (GetSecs - image_start_time >= Ladder2{runNum}(length(Ladder2{runNum}),1)/1000) && nocue % demo_go contains the information if a certain image is a GO trial or not
            ...
            end % end if  demo_go(Pos) == 11 && (GetSecs - image_start_time >= Ladder1{runNum}(length(Ladder1{runNum}),1)/1000) && nocue
        ...
        fprintf(fid1,'%s\t %d\t %s\t %d\t %d\t %d\t %d\t %d\t %d\t %.2f\t %d\t %d\n', subjectID, runNum, demoNames{Pos}, actual_onset_time{runNum}(Pos,1), demo_go(Pos), respTime{runNum}(Pos,1)*1000, respInTime{runNum}(Pos,1), cue_time{runNum}(Pos,1)*1000, keyPressed{runNum}(Pos,1),   fix_time{runNum}(Pos,1)-anchor, Ladder1{runNum}(length(Ladder1{runNum})), Ladder2{runNum}(length(Ladder2{runNum})));
...
clear demo_images Instructions_image CueImage CueImageAlpha;
