...
load("~/Dropbox/Boost_Israel/R/boost_isf_BDM.RData")
...
samp="ISF"
present=12
num=length(levels(as.factor(boost_isf_probe$subjid)))

#####PLOT WTP####

BDM=subset(boost_isf_BDM,boost_isf_BDM$PairType=="HHG"|boost_isf_BDM$PairType=="HHNG"|boost_isf_BDM$PairType=="LLG"|boost_isf_BDM$PairType=="LLNG")
...
m=tapply(boost_probe$Outcome2,list(boost_probe$subjid,boost_probe$PairType),mean,na.rm=T)

t1=glmer(Outcome2 ~ 1 + (1|subjid),data=subset(boost_probe,boost_probe$PairType==1),na.action=na.omit,family=binomial)
vc1 <- vcov(t1, useScale = FALSE)
b1 <- fixef(t1)
z1 <- b1 / sqrt(diag(vc1))
p1 <- 2 * (1 - pnorm(abs(z1)))
t2=glmer(Outcome2 ~ 1 + (1|subjid),data=subset(boost_probe,boost_probe$PairType==2),na.action=na.omit,family=binomial)
...
sd=c(tapply(boost_probe$Outcome2, boost_probe$PairType,sd,na.rm=T)[1],tapply(boost_probe$Outcome2, boost_probe$PairType,sd,na.rm=T)[2])
se1=std.error(m[,1])
se2=std.error(m[,2])
se=c(se1,se2)
