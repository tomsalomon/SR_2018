...
choices_files=dir([outpath '*bianry_ranking_*.txt']);

% needed to add this line because of the demo output files
demo_choices_files=dir([outpath '*demo_bianry_ranking_*.txt']);
for i=length(choices_files):-1:1
 if ismember((choices_files(i).name),{demo_choices_files(:).name}) 
     choices_files(i)=[];
 end
end
