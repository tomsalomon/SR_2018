...
for datafile = 1:length(logs)
    ...
    % In case some stimuli names are numeric and some are not (not all were
    % properly converted) - just replace with zero
    if length(Data{8})~=length(Data{1})||length(Data{9})~=length(Data{1})
        Data{8}=zeros(length(Data{1}),1); % left stimulus
        Data{9}=zeros(length(Data{1}),1); % right stimulus
    end
