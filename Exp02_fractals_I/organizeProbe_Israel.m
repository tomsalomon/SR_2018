...
% order = 1;
% test_comp = 4;
% mainPath = '/Users/schonberglabimac1/Documents/Boost_fractals_MAC';
% numRunsPerBlock = 2;
% block = 1;
...
switch order
    case 1
        ...
        % - - - - - - - - - - - - - - -
        sanityHV_beep = [4 5 24 25]; %HV_beep
        sanityLV_beep = [36 37 56 57]; %LV_beep
        
        
        sanityHV_nobeep = [3 6 23 26]; %HV_nobeep
        sanityLV_nobeep = [35 38 55 58]; %LV_nobeep
    ...
    case 2
        ...
        %   sanity check comparisons
        % - - - - - - - - - - - - - - -
        sanityHV_beep = [3 6 23 26]; %HV_beep
        sanityLV_beep = [35 38 55 58]; %LV_beep
        
        
        sanityHV_nobeep = [4 5 24 25]; %HV_nobeep
        sanityLV_nobeep = [36 37 56 57]; %LV_nobeep
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - -
numHighGoItems = length(HV_beep);
HV_beep_new = numHighGoItems^2;
HV_nobeep_new = numHighGoItems^2;
for i = 1:numHighGoItems
    for j = 1:numHighGoItems
        HV_beep_new(j+(i-1)*numHighGoItems) = HV_beep(i);
        HV_nobeep_new(j+(i-1)*numHighGoItems) = HV_nobeep(j);
    end
end
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - -
numLowGoItems = length(LV_beep);
LV_beep_new = numLowGoItems^2;
LV_nobeep_new = numLowGoItems^2;
for i = 1:numLowGoItems
    for j = 1:numLowGoItems
        LV_beep_new(j+(i-1)*numLowGoItems) = LV_beep(i);
        LV_nobeep_new(j+(i-1)*numLowGoItems) = LV_nobeep(j);
    end
end
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - -
numSanityGoItems = length(sanityHV_beep);
sanityHV_beep_new = numSanityGoItems^2;
sanityLV_beep_new = numSanityGoItems^2;
for i=1:numSanityGoItems
    for j=1:numSanityGoItems
        sanityHV_beep_new(j+(i-1)*numSanityGoItems) = sanityHV_beep(i);
        sanityLV_beep_new(j+(i-1)*numSanityGoItems) = sanityLV_beep(j);
    end
end
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - -
numSanityNoGoItems = length(sanityHV_nobeep);
sanityHV_nobeep_new = numSanityNoGoItems^2;
sanityLV_nobeep_new = numSanityNoGoItems^2;
for i = 1:numSanityNoGoItems
    for j = 1:numSanityNoGoItems
        sanityHV_nobeep_new(j+(i-1)*numSanityNoGoItems) = sanityHV_nobeep(i);
        sanityLV_nobeep_new(j+(i-1)*numSanityNoGoItems) = sanityLV_nobeep(j);
    end
end
...
%-----------------------------------------------------------------
numComparisons = numHighGoItems^2;
numSanity = numSanityGoItems^2;
