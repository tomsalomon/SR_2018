...
session2_outputPath = './../Output/recall/'; % write/correct ccording to the path of the second session results

logs2session=dir([session2_outputPath,'*_probe_block_04_run_02*.txt']);
...
for subjectInd = 1:length(logs2session)
    ...
    logs1session = dir([session1_outputPath,sub_name,'_probe_block*.txt']) ;
        
    NumDays = round(daysact(logs1session(1).date,  logs2session(subjectInd).date));
    diff_by_subject(:,1)=str2double(sub_name(end-2:end));
    diff_by_subject(subjectInd,2)=NumDays;
end
diff_by_subject(1,:)=[]; % remove sub 103 - should not be included
