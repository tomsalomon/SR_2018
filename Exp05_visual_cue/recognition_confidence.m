...
newStimName = dir([mainPath '/stim/recognitionNew/*.bmp']); % Read new stimuli
...
for i = 1:length(newStimName)
    imgArraysNew{i} = imread([mainPath '/stim/recognitionNew/' newStimName(i).name],'bmp');
end
...
% for trial = 1:6 % for debugging
  for trial = 1:length(stimName)
    ...
    Screen('PutImage',w, imgArrays{trial},rect); % display item
    Screen('TextSize',w, 40);
...
clear imgArrays imgArraysNew imgArraysOld answer_images recognition_answers_image newOldQuestion_Image goNoGoQuestion_Image;
save(outfile);
...
if test_comp == 1       
        ...
        CenterText(w,'Please call the experimenter', white,0,50);
        Screen(w,'Flip');
end
