...
% order = 1;
% mainPath =
% '/Users/schonberglabimac1/Documents/Boost_Israel_New_Rotem_mac'; % on
% experiment mac
% test_comp = 4;
...
for runNum = 1:numRuns
    ...
    for Pos = 1:length(demoNames) %length(stop{runNum}),   % To cover all the items in one run.
        ...
        while (GetSecs-image_start_time < image_duration)
            ...
            if  demo_go(Pos) == 11 && (GetSecs - image_start_time + 0.0174 >= Ladder1{runNum}(length(Ladder1{runNum}),1)/1000) && notone %demo_go contains the information if a certain image is a demo_go trial or not
                % "Visual Beep!"
                ...
                %---------------------------
            elseif  demo_go(Pos) == 22 && (GetSecs - image_start_time + 0.0174 >= Ladder2{runNum}(length(Ladder2{runNum}),1)/1000) && notone % demo_go contains the information if a certain image is a GO trial or not
