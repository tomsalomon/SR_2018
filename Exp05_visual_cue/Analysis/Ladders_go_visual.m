...
subjects=[101:116,118:125]; % Define here your subjects' codes.
%exclude:
% 117 - poor training
...
for subjInd=1:length(subjects)
    ...
    filename=strcat(outpath,sprintf('*_%d',subjects(subjInd)));
    logs=dir(strcat(filename, '_training_run','*.txt'));
    mats=dir(strcat(filename, '_training_run*.mat'));
    mats_train=dir(strcat(filename, 'training40_run','*.mat'));
