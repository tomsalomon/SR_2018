...
fprintf(fid1,'subjectID\torder\tdate\tgender(1-female, 2-male)\tage\tdominant hand (1-right, 2-left)\theight\tweight\toccupation\n'); %write the header line
...
Height = inputdlg ('Please type your height in Cm (for example 168): ','Height',1);
while isempty(Height) || isempty(Height{1})
    Height = inputdlg ('Please correct. enter your height: ','Height',1);
end
...
Weight = inputdlg ('Please type your weight in Kg (for example 58):','Weight',1);
while isempty(Weight) || isempty(Weight{1})
    Weight = inputdlg ('Please correct. enter your Weight: ','Weight',1);
end
...
fprintf(fid1,'%s\t%d\t%s\t%d\t%d\t%d\t%d\t%d\t%s\n', subjectID, order, timestamp, Gender, Age, DominantHand,Height, Weight, Occupation);
fclose(fid1);
end
