...
function binary_ranking(subjid,test_comp,stimuli,path)
%
%Change according to testing environment: the path that comes before
%"Dropbox\..."
% path=['C:\Documents and Settings\Tom Salomon\My Documents']; %laptop
% path=['E:\Tom\Dropbox\Experiment_Israel\Codes\Boost_Israel\Face_exp']; %lab
chosen_snack=[];
%=========================================================================
% Probe task code
%=========================================================================
...
% - - - - - - - - - - - - - - - - -
if stimuli==1;
    fid=fopen([path,'\Stimuli_List_snack.txt']);
    stimuli_type='snacks';
elseif stimuli==2
    fid=fopen([path,'\Stimuli_List_face.txt']);
        stimuli_type='faces';
        elseif stimuli==3
    fid=fopen([path,'\Stimuli_List_fractal.txt']);
...
for i=1:length(stimname)
    food_items{i}=imread(sprintf('%s',path,'\stim\',stimname{i}));
end
...
% IMPORTANT NOTE: every stimulus will be presented exactly 2*number_of_trials/n times. Therefore, number_of_trials must be a multiple of n/2. 
%=============                                                                                                                      ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
%-----------------------------------------------------------------
fid1=fopen([path,'\Output\' subjid '_' stimuli_type sprintf('_probe_ranking_'), timestamp '.txt'], 'a');
...
fid2=fopen([path,'\Output\' subjid '_' stimuli_type sprintf('_ItemRankingResults_'), timestamp '.txt'], 'a');
...
chosen_snack=Shuffle(chosen_snack);
imshow(food_items{chosen_snack(1)})
end
