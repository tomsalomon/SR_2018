function [] = probeDemo_Israel(subjectID, order, mainPath, test_comp, sessionNum, xlsfilename,stimuli)
...
if stimuli==1
    fid_demoNames=fopen('Misc/demo_items.txt','r+','s','windows-1252');
elseif stimuli==2
    fid_demoNames=fopen('Misc/Stimuli_List_face_demo.txt','r+','s','windows-1252');
elseif stimuli==3
    fid_demoNames=fopen('Misc/Stimuli_List_fractal_demo.txt','r+','s','windows-1252');
end
...
% onsetlist = load('Onset_files/probe_onset_4.mat');
...
fprintf(fid1,'subjectID scanner order runtrial actualOnsettime ImageLeft ImageRight TypeLeft TypeRight IsleftGo Response PairType Outcome RT \n'); %write the header line
...
for block=1:1
    for trial=1:4
        ...
        stimTime = 0;
        while stimTime == 0
            timeFromRunStart = GetSecs;
            if timeFromRunStart >= runStart+onsetlist(runtrial)
            Screen(w,'Flip');
            stimTime = 1;
            end
        end
        ...
        StimOnset = GetSecs;
        actualOnset(trial) = StimOnset - runStart;
        KbQueueStart;
        ...
        fprintf(fid1,'%s\t %d\t %d\t %d\t %d\t %s\t %s\t %d\t %d\t %d\t %s\t %d\t %d\t %.2f\t \n', subjectID, test_comp, order, runtrial, actualOnset(trial), char(leftname{block}(trial)), char(rightname{block}(trial)), stimnum1{block}(trial), stimnum2{block}(trial), leftGo{block}(trial), keyPressed, pairtype{block}(trial), out, respTime*1000);
        dataToSave = {runtrial, actualOnset(trial), char(leftname{block}(trial)), char(rightname{block}(trial)), stimnum1{block}(trial), stimnum2{block}(trial), leftGo{block}(trial), keyPressed, pairtype{block}(trial), out, respTime*1000};
