...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
%   [outputPath '/' subjectID '_training_demo_' timestamp '.txt']
%   Saves data to xls file created in the general 'run_boost_Israel_new'
%   function
...
for runNum = 1:numRuns %1:10
    ...
    %     oneSeveral{runNum}=vars{3};
    %     shuff_oneSeveral{runNum}=oneSeveral{runNum}(shuff_ind{runNum});
    ...
    for Pos=1:length(demoNames) %length(stop{runNum}),   % To cover all the items in one run.
        ...
        Screen('Flip',w,anchor+onsets(Pos)+runStartTime); % display images according to Onset times
        ...
        while (GetSecs-image_start_time < image_duration),
            ...
            if  demo_go(Pos)==22 && (GetSecs - image_start_time >=Ladder2{runNum}(length(Ladder2{runNum}),1)/1000) && notone % demo_go contains the information if a certain image is a GO trial or not
            ...
            if   mod(demo_go(Pos),11)~=0 && noresp % these will now be the NOGO trials
                [pressed, firstPress, ~, ~, ~] = KbQueueCheck;
                if pressed && noresp
                    findfirstPress=find(firstPress);
                    respTime{runNum}(Pos,1) = firstPress(findfirstPress(1))-image_start_time;
        ...
        switch respInTime{runNum}(Pos,1)
            ...
            case 11
                if (Ladder1{runNum}(length(Ladder1{runNum}),1)==1000); %was a GO trial with HV item but responded before SS make it harder - increase SSD by Step/3
                    Ladder1{runNum}(length(Ladder1{runNum})+1,1)=Ladder1{runNum}(length(Ladder1{runNum}),1);
                else
            ...
            case 22
                if (Ladder2{runNum}(length(Ladder2{runNum}),1)==1000); %was a GO trial with LV item but responded before SS make it harder - - increase SSD by Step/3
                    Ladder2{runNum}(length(Ladder2{runNum})+1,1)=Ladder2{runNum}(length(Ladder2{runNum}),1);
                else
            ...
            case 110 % pressed after Go signal but below 1000 HV item - - increase SSD by Step/3 - these are the good trials!
                if (Ladder1{runNum}(length(Ladder1{runNum}),1)==1000);
                    Ladder1{runNum}(length(Ladder1{runNum})+1,1)=Ladder1{runNum}(length(Ladder1{runNum}),1);
                else
                    Ladder1{runNum}(length(Ladder1{runNum})+1,1)=Ladder1{runNum}(length(Ladder1{runNum}),1)+Step/3;
                end;
            ...
            case 220 % pressed after Go signal but below 1000 LV item - - increase SSD by Step/3 - these are the good trials!
                if (Ladder2{runNum}(length(Ladder2{runNum}),1)==1000);
                    Ladder2{runNum}(length(Ladder2{runNum})+1,1)=Ladder2{runNum}(length(Ladder2{runNum}),1);
                else
                    Ladder2{runNum}(length(Ladder2{runNum})+1,1)=Ladder2{runNum}(length(Ladder2{runNum}),1)+Step/3;
                end;
        ...
        fprintf(fid1,'%d %d %d %d %d %d %.2f %d %d\n', actual_onset_time{runNum}(Pos,1), demo_go(Pos), respTime{runNum}(Pos,1)*1000, respInTime{runNum}(Pos,1), Audio_time{runNum}(Pos,1)*1000, keyPressed{runNum}(Pos,1),   fix_time{runNum}(Pos,1)-anchor, Ladder1{runNum}(length(Ladder1{runNum})), Ladder2{runNum}(length(Ladder2{runNum})));
        dataToSave = {Pos, demoNames{Pos}, actual_onset_time{runNum}(Pos,1), demo_go(Pos), respTime{runNum}(Pos,1)*1000, respInTime{runNum}(Pos,1), Audio_time{runNum}(Pos,1)*1000, keyPressed{runNum}(Pos,1),   fix_time{runNum}(Pos,1)-anchor, Ladder1{runNum}(length(Ladder1{runNum})), Ladder2{runNum}(length(Ladder2{runNum}))};
        dataRange = ['A' num2str(6+Pos)];
