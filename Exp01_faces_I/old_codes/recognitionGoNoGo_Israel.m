...
for i = 1:numStimuli
    if oldIsOld(i) == 1
        imgArrays{i} = imread([mainPath '/stim/' oldStimName{i}],'bmp');
    else
        imgArrays{i} = imread([mainPath '/stim/recognitionNew/' oldStimName{i}],'bmp');
    end
end
...
fid1 = fopen([outputPath '/' subjectID '_recognitionoGoNoGo' num2str(sessionNum) '_' timestamp '.txt'], 'a');
