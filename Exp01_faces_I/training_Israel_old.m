function [] = training_Israel(subjectID,order,mainPath,test_comp,runInd,total_num_runs)
...
% This session is composed of total_num_runs number of runs, two runs in
% each time this function runs (runInd, runInd+1)
...
%   ''\Onset_files\train_onset_' num2str(r(1)) '.mat''  where r=1-4
...
bidValues = cell(1,total_num_runs);
shuff_bidValues = cell(1,total_num_runs);
...
for runNum = runInd:runInd+1 %this for loop only allows 2 runs to be completed
    ...
    %---------------------------
    r = Shuffle(1:4);
    load(['Onset_files\train_onset_' num2str(r(1)) '.mat']);
    ...
    for i = 1:length(shuff_names{runNum})
        food_items{i} = imread(['stim\',sprintf('%s',shuff_names{runNum}{i})]); %CHANGED HERE
    end
...
%     for trialNum=1:length(shuff_trialType{runNum})/10; % shorter version for debugging
...
%         
    ...
    numGoTrials = length(find(trialType{runNum} == 11 | trialType{runNum} == 22));
    ...
    switch firstOrSecond
        case 0
            ...
            if correct{runNum}(1) < numGoTrials/2
...
% ------------------------------
fid2 = fopen([outputPath '\' subjectID sprintf('_ladders_run_%d.txt', runInd)], 'w');
