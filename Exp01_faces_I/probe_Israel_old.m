function probe_Israel_old(subjectID, order, mainPath, test_comp, sessionNum, block, numRun, numRunsPerBlock, trialsPerRun)

% function probe_Israel(subjectID, order, mainPath, test_comp, sessionNum, block, numRun, numRunsPerBlock, trialsPerRun, xlsfilename)
%
% = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
...
% 
%   Also saves data to xls file created in the general 'run_boost_Israel_new'
%   function: ''probe_results_' probeType '_' timestamp'
...
pixelSize=32;
% [w] = Screen('OpenWindow',screennum,[],[0 0 1040 880],pixelSize/4); %debugging screensize
...
fid = fopen([outputPath '/' subjectID sprintf('%s_stimuliForProbe_order%d_block_%d_run%d.txt',subjectID,order,block,numRun)]);
...
fid1 = fopen([outputPath '\' subjectID sprintf('faces_probe_block_%02d_', block) timestamp '.txt'], 'a');
fprintf(fid1,'subjectID\t scanner\t order\t block\t run\t trial\t onsettime\t ImageLeft\t ImageRight\t bidIndexLeft\t bidIndexRight\t IsleftGo\t Response\t PairType\t Outcome\t RT\t bidIndexLeft\t bidIndexRight\t bidLeft\t bidRight \n'); %write the header line
...
if test_comp==1
    ...
    if numRun == 1 % If this is the first run of the block, show instructions
        Screen('TextSize',w, 60);
        if block == 1 || block == 2
...
%     KbQueueStart;
    ...
    %-----------------------------------------------------------------
    if leftGo(trial)==1
        fprintf(fid1,'%s\t %d\t %d\t %s\t %d\t %d\t %d\t %s\t %s\t %d\t %d\t %d\t %s\t %d\t %d\t %.2f\t %d\t %d\t %.2f\t %.2f \n', subjectID, test_comp, order, sprintf('%02d', block), numRun, runtrial, StimOnset-runStart, char(leftname(trial)), char(rightname(trial)), stimnum1(trial), stimnum2(trial), leftGo(trial), keyPressed, pairType(trial), out, respTime*1000, bidIndex(stimnum1(trial)),bidIndex(stimnum2(trial)),bidValue(stimnum1(trial)), bidValue(stimnum2(trial)));
    else
        fprintf(fid1,'%s\t %d\t %d\t %s\t %d\t %d\t %d\t %s\t %s\t %d\t %d\t %d\t %s\t %d\t %d\t %.2f\t %d\t %d\t %.2f\t %.2f \n', subjectID, test_comp, order, sprintf('%02d', block), numRun, runtrial, StimOnset-runStart, char(leftname(trial)), char(rightname(trial)), stimnum2(trial), stimnum1(trial), leftGo(trial), keyPressed, pairType(trial), out, respTime*1000, bidIndex(stimnum2(trial)),bidIndex(stimnum1(trial)),bidValue(stimnum2(trial)), bidValue(stimnum1(trial)));
    end
...
%---------------------------------------------------------------
outfile = strcat(outputPath,'/', sprintf('%s_faces_probe_block_%2d_run_%2d_%s.mat',subjectID,block,numRun,timestamp));
