% function [probe_results,allSubjectsData] = probeAnalysis_Israel_new(mainPath, experimentName, Subjects)
function [probe_results,allSubjectsData, Recognition_results] = probeAnalysis_Israel_new(mainPath, experimentName, Subjects)
...
if nargin < 3
    Subjects = [101:105 107:109 111:114 116:121]; % write here only the subjects that should be analyzed (were not excluded)
    % 102- excluded due to extremely low rankings in the BDM
    % 108 - excluded due to low ladders (0...)
end
...
for subjectInd = 1:length(Subjects)
    ...
    subjectData = joinProbe(outputPath,experimentName,Subjects(subjectInd));
...
Recognition_results = recognition_analysis(mainPath, experimentName, Subjects);
