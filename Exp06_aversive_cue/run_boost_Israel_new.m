...
test_comp = input('Which computer are you using? 4 new mac 3 Rotem_PC 2 Toms_iMac, 1 MRI, 0 if testooom: ');
while isempty(test_comp) || sum(okComputer==test_comp)~=1
    disp('ERROR: input must be 0,1,2,3 or 4. Please try again.');
    test_comp = input('Which computer are you using? 4 new mac 3 Rotem_PC, 2 Toms_iMac, 1 MRI, 0 if testooom: ');
end
...
switch test_comp
    case 0
        mainPath = '~\Documents\Boost_Short\Output'; % Change when we have the new computer for the behavioral task
    case 1
    ...
    case 4
        mainPath = '/Users/ranigera/Desktop/Boost_Only_VIsual';
end % end switch
